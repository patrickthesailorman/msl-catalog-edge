...
public class ArtistMockData {
  ...
  /**
   * Applies mocked facet to a list of Artists
   *
   * @param facets String[]
    ...
    for (String facet : facets) {
      if (!FacetServiceFactory.isRatingFacet(facet, facetMockData.getRatingFacets())) {
        ...
        for (ArtistInfo artist : artistList) {
          String facetName =
              FacetServiceFactory.getFacet(facet, facetMockData.mockFacets).getName();
  ...
  /**
   * Apply mocked ratings facet to the artists
   *
   * @param facets String[]
   * @param artistList List<ArtistInfo>
   * @return List<ArtistInfo>
  ...
  /**
   * Browse through artists to get a subset matching specified filters
   *
  ...
  public ArtistMockData() {
    ...
    artistMockData1
        .setImageLink("https://veggiesrock.files.wordpress.com/2011/07/vai2009_promo11.jpg");
    ...
    artistMockData7
        .setImageLink("http://www.spirit-of-metal.com/membre_groupe/photo/Joe_Satriani-14020.jpg");
